(function(ns){
	...
	var $regexs = {
		...
		integer_any : /^[-\+]?\d+$/,
		double_any : /^[-\+]?\d+(\.\d+)?$/,
		phone_mobile : /^((((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?)|((\(\d{2,3}\))|(\d{3}\-))?((13\d{9})|(15\d{9})|(18\d{9})))$/
	};
	...
	var Validator = ns.Validator = {
		...
		toString : function(){return ['Version: 4.0.2 beta', 'Author: \u6211\u4f5b\u5c71\u4eba', 'Email: wfsr-at-msn.com'].join('\n');}
	};
	...
	var AbstractElement = new Class({
		...
		focus : function(){
			if (this.element.className.indexOf("Wdate") >= 0){
				return;
			}
		...
		getMessagePanel : function(){
			if(this.element==null||this.element.name==null){
				return this.panel;
			}
