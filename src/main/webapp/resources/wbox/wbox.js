...
(function(jQuery){
    ...
    jQuery.fn.wBox = function(options){
        var defaults = {
            wBoxURL: "wbox/",
            opacity: 0.5,//背景透明度
            callBack: null,
		...
		this.YQ = jQuery.extend(defaults, options);
        ...
        '<div class="wBox_content" id="wBoxContent"></div></div></td><td class="wBox_b"><div style="width:10px;">&nbsp;</div></td></tr><tr><td class="wBox_bl"/><td class="wBox_b"/><td class="wBox_br"/></tr></tbody></table></div></div>', B = null, C = null, jQuerywin = jQuery(window),jQueryt=jQuery(this);//B背景，C内容jquery div   
        this.showBox=function (){
            jQuery("#wBox_overlay").remove();
			jQuery("#wBox").remove();
            
            B = jQuery("<div id='wBox_overlay' class='wBox_hide'></div>").hide().addClass('wBox_overlayBG').css('opacity', _this.YQ.opacity).dblclick(function(){
                _this.close();
            }).appendTo('body').fadeIn(300);
            C = jQuery(wBoxHtml).appendTo('body');
            handleClick();
        }
        /*
         * 处理点击
        ...
        function handleClick(){
			...
			if (_this.YQ.requestType && jQuery.inArray(_this.YQ.requestType, ['iframe', 'ajax','img'])!=-1) {
				...
				if (_this.YQ.requestType === "img") {
					...
					if (_this.YQ.requestType === "ajax") {
						jQuery.get(_this.YQ.target, function(data){
							con.html(data);
						...
						ifr = jQuery("<iframe name='wBoxIframe' style='width:" + _this.YQ.iframeWH.width + "px;height:" + _this.YQ.iframeWH.height + "px;' scrolling='auto' frameborder='0' src='" + _this.YQ.target + "'></iframe>");
						ifr.appendTo(con.empty());
						ifr.load(function(){
							try {
								jQueryit = jQuery(this).contents();
								jQueryit.find('.wBox_close').click(_this.close);
								fH = jQueryit.height();//iframe height
								fW = jQueryit.width();
								w = jQuerywin;
				...
				if (_this.YQ.target) {
					jQuery(_this.YQ.target).clone(true).show().appendTo(con.empty());
					...
					if (_this.YQ.html) {
						...
						jQueryt.clone(true).show().appendTo(con.empty());
					}         
            afterHandleClick();
        }
        /*
        ...
        function afterHandleClick(){     
            ...
            jQuery(document).unbind('keydown.wBox').bind('keydown.wBox', function(e){
                if (e.keyCode === 27) 
                    _this.close();
                return true
            });
        ...
        function setPosition(){
            ...
            var jQueryh = jQuery("body").height(), jQuerywh = jQuerywin.height(),jQueryhh=jQuery("html").height();
            jQueryh = Math.max(jQueryh, jQuerywh);
            B.height(jQueryh).width(jQuerywin.width())            
        }
        /*
         * 计算wBox的位置
         * @param {number} w 宽度
        ...
        function calPosition(w){
            l = (jQuerywin.width() - w) / 2;
            t = jQuerywin.scrollTop() + jQuerywin.height() /9;
            return [l, t];
        }
        /*
         * 拖拽函数drag
        ...
        function drag(){
            ...
            function move(e){
                ...
                if (e.clientX - dx < 0) {
                    ...
                    if (e.clientX - dx > jQuerywin.width() - C.width()) {
                        l = jQuerywin.width() - C.width();
                    }
                    else {
                        l = e.clientX - dx
                    }
         ...
         * 触发click事件
         */		
        jQuerywin.resize(function(){
            setPosition();
        });
		_this.YQ.show?_this.showBox():jQueryt.click(function(){
            _this.showBox();
            return false;
        });
		return this;
