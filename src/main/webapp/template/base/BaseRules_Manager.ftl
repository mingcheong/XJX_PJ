...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="topgzq" height="28" align="center">
    <tr>
    	<td align="center" ><div class="dqwz">您现在的位置：法律法规管理</div></td>
    </tr>
</table>
<@s.form id="baseRulesForm" action="baseRules_manager.xhtml"><@s.token />
    ...
    <td class="queryTable" align="center">
		<table width="90%" border="0" cellpadding="0" cellspacing="0" id="queryTable" rules="none" frame="void">
			<tr>
				<th width="15%">标题</th>
		        <td>
		        	<@s.textfield id="name" name="baseRulesModel.title" cssStyle="width:150px"/>
		        </td>
				<th width="15%">类型</th>
		        <td>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_menu" height="30" align="center">
  <tr>
	...
	<div >
	  <div class="menu_left">
		<ul>
			<@util.menu_add '添 加'> obj="baseRules" </@util.menu_add>
			<@util.menu_edit '编 辑'> obj="baseRules" </@util.menu_edit>
			<@util.menu_delAll '删 除'> obj="baseRules" </@util.menu_delAll>
	...
	<@jmesa.htmlTable width="100%" style="text-align:left;">
		<@jmesa.htmlRow>
				<@jmesa.htmlColumn property="id" style="text-align: center;" width="3%" title="<input type='checkbox' id='checkAll' name='checkAll'/>" filterable=false sortable=false>
					<input type="checkbox" name="selectedIds" value="${baseRules.id}" />
				</@jmesa.htmlColumn>
				<@jmesa.htmlColumn property="name" title="标题" ><a href="${contextPath}/baseRules_detail.xhtml?id=${baseRules.id}">${baseRules.title}</a></@jmesa.htmlColumn>
...
<#if baseRulesModel.rulesType?if_exists != "">jQuery("#rules_type").val("${baseRulesModel.rulesType?default('')}");</#if>
