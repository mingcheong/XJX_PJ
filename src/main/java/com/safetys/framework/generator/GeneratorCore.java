...
import com.safetys.framework.generator.utils.FileHelper;
import com.safetys.framework.generator.utils.IOHelper;
import com.safetys.framework.generator.utils.PropertiesProvider;
import com.safetys.framework.generator.utils.StringTemplate;
		...
		for (int i = 0; i < templateFiles.size(); i++) {
			...
			try {
				...
				if (templateFile.getName().toLowerCase().endsWith(".include")) {
					System.out.println("[跳过]\t 扩展名为 '.include' 的模型文件:" + templateRelativePath);
					continue;
				}
				...
				if ((testExpressionIndex = templateRelativePath.indexOf('@')) != -1) {
					...
					if (!"true".equals(expressionValue.toString())) {
						System.out.println("[未生成]\t 测试异常 '@" + testExpressionKey + "' 为假,模板:" + templateRelativePath);
						continue;
					}
				}
	...
	private void generateFile(IModel model, Configuration config, String templateRelativePath, String outputFilePath) throws Exception {
		...
		if (absoluteOutputFilePath.exists()) {
			...
			if (isFoundInsertLocation(template, templateDataModel, absoluteOutputFilePath, newFileContentCollector)) {
				System.out.println("[插入]\t 添加内容到:" + targetFilename);
