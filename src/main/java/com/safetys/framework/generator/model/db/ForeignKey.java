...
import com.safetys.framework.generator.utils.ListHashtable;
...
public class ForeignKey {
	...
	private void initRelationship() {
		...
		try {
			foreignTable = (Table) DbModelHelper.getInstance().getTable(tableName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (hasAllPrimaryKeys(parentPrimaryKeys, parentColumns)) firstRelation = "one";
		else firstRelation = "many";

		if (hasAllPrimaryKeys(foreignPrimaryKeys, columns)) secondRelation = "one";
		else secondRelation = "many";
	...
	public String getFirstRelation() {
		if (firstRelation == null) initRelationship();
	...
	public Table getSqlTable() {
		Table table = null;
		try {
			table = (Table) DbModelHelper.getInstance().getTable(tableName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return table;
	...
	public String getRelationShip() {
		if (relationShip == null) initRelationship();
		return relationShip;
	}
	...
	public String getSecondRelation() {
		if (secondRelation == null) initRelationship();
