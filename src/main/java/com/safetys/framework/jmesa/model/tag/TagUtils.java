...
import com.safetys.framework.jmesa.limit.Order;
...
import com.safetys.framework.jmesa.view.editor.BasicCellEditor;
...
import com.safetys.framework.jmesa.view.html.event.RowEvent;
...
import com.safetys.framework.jmesa.view.html.renderer.HtmlRowRenderer;
import com.safetys.framework.jmesa.view.html.renderer.HtmlTableRenderer;
...
class TagUtils {
	...
	static boolean validateColumn(SimpleTagSupport simpleTagSupport, String property) {
		if (property == null) { return true; // no coflicts
		}
	...
	public static int[] getTableFacadeMaxRowIncrements(String maxRowsIncrements) {
		if (StringUtils.isEmpty(maxRowsIncrements)) { return null; }
	...
	static HtmlTableRenderer getTableTableRenderer(HtmlTable table, String tableRenderer) {
		if (StringUtils.isBlank(tableRenderer)) { return table.getTableRenderer(); }
	...
	static HtmlRowRenderer getRowRowRenderer(HtmlRow row, String rowRenderer) {
		if (StringUtils.isBlank(rowRenderer)) { return row.getRowRenderer(); }
	...
	static RowEvent getRowOnclick(HtmlRow row, String onclick) {
		if (StringUtils.isBlank(onclick)) { return row.getOnclick(); }
	...
	static RowEvent getRowOnmouseover(HtmlRow row, String onmouseover) {
		if (StringUtils.isBlank(onmouseover)) { return row.getOnmouseover(); }
	...
	static RowEvent getRowOnmouseout(HtmlRow row, String onmouseout) {
		if (StringUtils.isBlank(onmouseout)) { return row.getOnmouseout(); }
	...
	static HtmlCellRenderer getColumnCellRenderer(HtmlColumn column, String cellRenderer) {
		if (StringUtils.isBlank(cellRenderer)) { return column.getCellRenderer(); }
	 ...
	 * </p>
	 * <p>
	 * If it is defined and it extends ContextSupport then set the WebContext
	 * and CoreContext on the worksheetEditor.
	 * </p>
	 * 
	...
	static WorksheetEditor getColumnWorksheetEditor(HtmlColumn column, String worksheetEditor) {
		if (StringUtils.isEmpty(worksheetEditor)) { return null; }
	 ...
	 * <p>
	 * If it is defined and it extends ContextSupport then set the WebContext
	 * and CoreContext on the editor. If a setPattern() method is defined on
	 * your editor and you have defined the column pattern attribute on the tag
	 * it will be set on your CellEditor automatically.
	 * </p>
	 * 
	...
	static CellEditor getColumnCellEditor(HtmlColumn column, String cellEditor, String pattern, boolean hasBody) {
		...
		} else if (cellEditorNotDefined) { return new HtmlCellEditor(); }
	...
	static HtmlFilterRenderer getColumnFilterRenderer(HtmlColumn column, String filterRenderer) {
		if (StringUtils.isBlank(filterRenderer)) { return column.getFilterRenderer(); }
	...
	static HtmlHeaderRenderer getColumnHeaderRenderer(HtmlColumn column, String headerRenderer) {
		if (StringUtils.isBlank(headerRenderer)) { return column.getHeaderRenderer(); }
	...
	static HeaderEditor getColumnHeaderEditor(HtmlColumn column, String headerEditor) {
		if (StringUtils.isEmpty(headerEditor)) { return column.getHeaderEditor(); }
	...
	/**
	 * <p>
	 * Get a Map of name-value pairs for an input string having format
	 * "name1:value1;[name2:value2];..."
	 * </p>
	 */
	static Map<String, String> getNameValueMap(String input, String errorMessage) {
		if (StringUtils.isEmpty(input)) { return Collections.emptyMap(); }

		Map<String, String> nameValueMap = new HashMap<String, String>();

		for (String nameValue : input.split(";")) {
			String nameValueParams[] = nameValue.split(":");
			if (nameValueParams.length != 2) { throw new IllegalArgumentException(errorMessage); }
	 ...
	 * </p>
	 */
	static List<WorksheetValidation> getWorksheetValidations(HtmlColumn column, String worksheetValidation, String errorMessageKey, String errorMessage, boolean custom) {

		if (StringUtils.isEmpty(worksheetValidation)) { return Collections.emptyList(); }

		List<WorksheetValidation> results = new ArrayList<WorksheetValidation>();

		Map<String, String> errorMessageKeys = getNameValueMap(errorMessageKey, "Required format of errorMessageKey is \"validationType:error message key;[second pair]\"");

		Map<String, String> errorMessages = getNameValueMap(errorMessage, "Required format of errorMessage is \"validationType:error message;[second pair]\"");
	...
	/**
	 * Take the comma separted Order values and convert them into an Array. The
	 * legal values include "none, asc, desc".
	 * 
	 * @since 2.2
	 * @return The sort order array for the column.
	 */
	public static Order[] getColumnSortOrder(String sortOrder) {
		if (StringUtils.isBlank(sortOrder)) { return null; }
		...
		for (int i = 0; i < orders.length; i++) {
			...
			if (valueOfParam == null) { throw new IllegalStateException("The sortOrder must consist of comma separated values of asc, desc, and none."); }
	...
	public static ExportType[] getTableFacadeExportTypes(String exportTypes) {
		if (StringUtils.isBlank(exportTypes)) { return null; }
