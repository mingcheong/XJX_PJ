...
import static com.safetys.framework.jmesa.model.tag.TagUtils.getTableFacadeFilterMatcherMap;
import static com.safetys.framework.jmesa.model.tag.TagUtils.getTableFacadeMessages;
import static com.safetys.framework.jmesa.model.tag.TagUtils.getTableFacadePreferences;
import static com.safetys.framework.jmesa.model.tag.TagUtils.getTableFacadeRowFilter;
import static com.safetys.framework.jmesa.model.tag.TagUtils.getTableFacadeColumnSort;
import static com.safetys.framework.jmesa.model.tag.TagUtils.getTableFacadeToolbar;
import static com.safetys.framework.jmesa.model.tag.TagUtils.getTableFacadeMaxRowIncrements;
import static com.safetys.framework.jmesa.model.tag.TagUtils.getTableFacadeView;
import static com.safetys.framework.jmesa.model.tag.TagUtils.getTableFacadeState;
import static com.safetys.framework.jmesa.model.tag.TagUtils.getTableFacadeExportTypes;
...
import com.safetys.framework.jmesa.facade.TableFacadeFactory;
import com.safetys.framework.jmesa.limit.Limit;
import static com.safetys.framework.jmesa.model.TableModelUtils.LIMIT_ATTR;
import com.safetys.framework.jmesa.view.View;
import com.safetys.framework.jmesa.view.ViewUtils;
import com.safetys.framework.jmesa.view.html.HtmlComponentFactory;
import com.safetys.framework.jmesa.view.html.HtmlUtils;
import com.safetys.framework.jmesa.view.html.component.HtmlTable;
import com.safetys.framework.jmesa.web.JspPageWebContext;
import com.safetys.framework.jmesa.web.WebContext;
import com.safetys.framework.jmesa.worksheet.Worksheet;
import com.safetys.framework.utils.TagLibWriter;

/**
 * A tag abstraction similar to the TableFacade. See the TableFacade document
...
public class TableModelTag extends SimpleTagSupport {
	...
	/**
	 * The unique identifier for this table. If you are using the stateAttr
	 * (State feature) then be sure that this value is unique.
	 */
	...
	/**
	 * Set the State on the facade. This will override the State if it was
	 * previously set.
	 * 
	 * @param state
	 *            The State to use.
	 ...
	 * 
	 * @param stateAttr
	 *            The parameter that will be searched to see if the state should
	 *            be used.
	 */
	...
	/**
	 * @return Is true if should sort and filter the Collection of Beans (or
	 *         Maps).
	 */
	...
	/**
	 * By default the facade will sort and filter the Collection of Beans (or
	 * Maps) automatically. This should be set to false if you are handling the
	 * filtering and sorting of the Collection automatically.
	 * 
	...
	/**
	 * the comma separated list of export types. The currently supported types
	 * are TableFacadeImpl.CVS and TableFacadeImpl.EXCEL.
	 */
	 ...
	 * 
	 * @param editable
	 *            Is true if the table should be editable.
	 */
	...
	/**
	 * Get the Messages. If the Messages does not exist then one will be
	 * created.
	 * 
	...
	/**
	 * Get the Preferences. If the Preferences does not exist then one will be
	 * created.
	 * 
	...
	/**
	 * Get the RowFilter. If the RowFilter does not exist then one will be
	 * created.
	 * 
	...
	/**
	 * Get the ColumnSort. If the ColumnSort does not exist then one will be
	 * created.
	 * 
	 ...
	 * 
	 * @param filterMatcherMap
	 *            The Map of current FilterMatchers.
	 */
	...
	/**
	 * These are the converted items. What happens is the regular items are
	 * passed to the columns, but then the columns are resolved with the EL
	 * Expression Language. A new Map is swapped out for each row Bean (or Map)
	 * and then used in place of the regular items. So in effect this is a
	 * Collection of Maps that represents the original Collection of Beans (or
	 * Maps) after being processed by the columns.
	 * 
	...
	/**
	 * Process the table, rows, and columns. Will pass the current bean in the
	 * pageScope for each bean in the Collection of Beans or Collection of Maps.
	 */
	@Override
	public void doTag() throws JspException, IOException {
		JspFragment body = getJspBody();
		if (body == null) { throw new IllegalStateException("You need to wrap the table in the facade tag."); }
