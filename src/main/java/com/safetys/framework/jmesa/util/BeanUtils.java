	...
	public static Method getReadMethod(Class<?> clazz, String property) {
		...
		for (String prefix : getterPrefixes) {
			...
			try {
				...
				logger.debug("No such read method '" + methodName + "()' in class '" + clazz.getName() + "'.");
			} catch (SecurityException e) {
				// do nothing but logging
				logger.debug("Error occurs while getting read method '" + methodName + "()' in class '" + clazz.getName() + "'.");
			}
		}
