 ...
 * the License.
 */
package com.safetys.framework.jmesa.view.pdfp;
...
import static com.safetys.framework.jmesa.view.ExportConstants.PDF_FONT_ENCODING;
import static com.safetys.framework.jmesa.view.ExportConstants.PDF_FONT_NAME;
import static com.safetys.framework.jmesa.view.ViewUtils.isRowEven;
	...
	private Font getFontWithColor(Color color) {
		...
		if (isNotBlank(fontName) && isNotBlank(fontEncoding)) {
			try {
				...
				if (color != null) { return new Font(baseFont, 12, 0, color); }
		...
		if (color != null) { return getFont(HELVETICA, 12, NORMAL, color); }
