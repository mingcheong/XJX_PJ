 ...
 */
package com.safetys.framework.fckeditor.response;

import com.safetys.framework.fckeditor.localization.LocalizedMessages;
import com.safetys.framework.fckeditor.requestcycle.ThreadLocalData;
import com.safetys.framework.fckeditor.tool.Utils;

/**
 * Represents the <a href="http://docs.fckeditor.net/FCKeditor_2.x/Developers_Guide/Server_Side_Integration#FileUpload_.28HTML.29" target="_blank">HTML/JS response</a> for the File Browser
 * <code>POST</code> requests.
 * <p>
 * This class utilizes varags to reflect the JavaScript callback function in an optimal way. However, varargs can be tricky. You can always omit passed parameters from right to left but any violation
 * may result in an exception or a failed callback.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * But <code>window.parent.OnUploadCompleted(101,'some/url/file.img','no error')</code> is an illegal callback and will fail.
 * </p>
 * 
...
public class UploadResponse {
	 ...
	 * <p>
	 * Use the predefined error numbers or upload responses, if possible. If you need to set error number and message only, use this constructor with the first argument only and call
	 * {@link UploadResponse#setCustomMessage(String)}.
	 * </p>
	 * 
	 * @param arguments
	 *            possible argument order: <code>int errorNumber, String fileUrl, String filename, String customMessage</code> . Omit from right to left.
	...
	public UploadResponse(Object... arguments) {
		if ((arguments.length < 1) || (arguments.length > 4)) { throw new IllegalArgumentException("The amount of arguments has to be between 1 and 4"); }

		this.parameters = new Object[arguments.length];

		if (!(arguments[0] instanceof Integer)) { throw new IllegalArgumentException("The first argument has to be an error number (int)"); }
	...
	public void setCustomMessage(final String customMassage) {
		if (Utils.isNotEmpty(customMassage)) {
			if (this.parameters.length == 1) {
				final Object errorNumber = this.parameters[0];
	...
	public static UploadResponse getOK(String fileUrl) {
		return new UploadResponse(UploadResponse.EN_OK, fileUrl);
	}
	...
	public static UploadResponse getFileRenamedWarning(String fileUrl, String newFileName) {
		...
		return new UploadResponse(UploadResponse.EN_FILE_RENAMED_WARNING, fileUrl, newFileName, lm.getFileRenamedWarning(newFileName));
	}

	/** Creates a <code>INVALID FILE TYPE</code> error. */
	public static UploadResponse getInvalidFileTypeError() {
		final LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData.getRequest());
		return new UploadResponse(UploadResponse.EN_INVALID_FILE_TYPE_ERROR, lm.getInvalidFileTypeSpecified());
	}

	/** Creates a <code>INVALID COMMAND</code> error. */
	public static UploadResponse getInvalidCommandError() {
		final LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData.getRequest());
		return new UploadResponse(UploadResponse.EN_CUSTOM_ERROR, null, null, lm.getInvalidCommandSpecified());
	}

	/** Creates a <code>INVALID RESOURCE TYPE</code> error. */
	public static UploadResponse getInvalidResourceTypeError() {
		final LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData.getRequest());
		return new UploadResponse(UploadResponse.EN_CUSTOM_ERROR, null, null, lm.getInvalidResouceTypeSpecified());
	}

	/** Creates a <code>INVALID CURRENT FOLDER</code> error. */
	public static UploadResponse getInvalidCurrentFolderError() {
		final LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData.getRequest());
		return new UploadResponse(UploadResponse.EN_CUSTOM_ERROR, null, null, lm.getInvalidCurrentFolderSpecified());
	}

	/** Creates a <code>FILE UPLOAD DISABLED</code> error. */
	public static UploadResponse getFileUploadDisabledError() {
		final LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData.getRequest());
		return new UploadResponse(UploadResponse.EN_SECURITY_ERROR, null, null, lm.getFileUploadDisabled());
	}

	/** Creates a <code>FILE UPLOAD WRITE</code> error. */
	public static UploadResponse getFileUploadWriteError() {
		final LocalizedMessages lm = LocalizedMessages.getInstance(ThreadLocalData.getRequest());
		return new UploadResponse(UploadResponse.EN_CUSTOM_ERROR, null, null, lm.getFileUploadWriteError());
	}
