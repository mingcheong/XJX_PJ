...
import com.safetys.framework.fckeditor.requestcycle.Context;
import com.safetys.framework.fckeditor.requestcycle.ThreadLocalData;
import com.safetys.framework.fckeditor.response.GetResponse;
import com.safetys.framework.fckeditor.response.UploadResponse;
import com.safetys.framework.fckeditor.tool.Utils;
import com.safetys.framework.fckeditor.tool.UtilsFile;
import com.safetys.framework.fckeditor.tool.UtilsResponse;
 ...
 * connector servlet. More over, it intercepts all {@link com.safetys.framework.fckeditor.connector.exception specified exceptions} from a connector and emits appropriate (localized) messages to the user. The
...
public class Dispatcher {
	 ...
	 */
	private GetResponse getFoldersAndOrFiles(final Command command, final ResourceType type, final String currentFolder, final String constructedUrl) throws InvalidCurrentFolderException,
			ReadException {
		...
		if (!RequestCycleHandler.isFileUploadEnabled(request)) {
			...
			try {
				...
				if (type.isDeniedExtension(FilenameUtils.getExtension(fileName))) {
					...
					final String newFileName = this.connector.fileUpload(type, context.getCurrentFolderStr(), sanitizedFileName, uplFile.getInputStream());
					final String fileUrl = UtilsResponse.fileUrl(RequestCycleHandler.getUserFilesPath(request), type, context.getCurrentFolderStr(), newFileName);
