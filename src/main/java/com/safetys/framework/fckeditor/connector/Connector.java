 ...
 */
package com.safetys.framework.fckeditor.connector;
...
import com.safetys.framework.fckeditor.connector.exception.ReadException;
import com.safetys.framework.fckeditor.connector.exception.WriteException;
import com.safetys.framework.fckeditor.connector.impl.AbstractLocalFileSystemConnector;
...
/**
 * Backend interface of a File Browser connector. A connector serves and manages files and folders accessed through the File Browser on an arbitrary backend system.<br />
 * The connector will receive a request if, and only if, the request was valid in terms of valid and reasonable parameters up to an abstract point which is independent from a specific connector
 * implementation.
 * <p>
 * Helpful classes and methods:
 * <ol>
 * <li>If you need to access the request instance itself and/or the context parameters sent from the File Browser, take a look at the {@link ThreadLocalData} class.</li>
 * <li>Use {@link RequestCycleHandler#getUserFilesAbsolutePath(javax.servlet.http.HttpServletRequest) RequestCycleHandler.getUserFilesAbsolutePath},
 * {@link AbstractLocalFileSystemConnector#getRealUserFilesAbsolutePath(java.lang.String) AbstractLocalFileSystemConnector.getRealUserFilesAbsolutePath} (if use it) to resolve the real path or simply
 * do it yourself.</li>
 * </ol>
 * </p>
 * 
...
public interface Connector {
	...
	/**
	 * Initializes this connector. Called at {@link Dispatcher dispatcher} initialization.
	 * 
	...
	/**
	 * Returns a list of file attributes from the backend. Use the pre-defined keys ({@value #KEY_NAME}, {@value #KEY_SIZE}) to put file attributes into the file map. The file length can be any
	 * instance of {@link Number}, its long value will be taken as the final value.
	 * 
	 * @param type
	 ...
	 */
	public List<Map<String, Object>> getFiles(final ResourceType type, final String currentFolder) throws InvalidCurrentFolderException, ReadException;
	 ...
	 */
	public List<String> getFolders(final ResourceType type, final String currentFolder) throws InvalidCurrentFolderException, ReadException;
	...
	public void createFolder(final ResourceType type, final String currentFolder, final String newFolder) throws InvalidCurrentFolderException, InvalidNewFolderNameException,
			FolderAlreadyExistsException, WriteException;
