...
import com.safetys.framework.fckeditor.requestcycle.impl.ContextPathBuilder;
 ...
 * <ul>
 * <li>Stored files and folders will be gone if the context is undeployed, unless you have saved them before.</li>
 * <li>Some servers are unable to write to the real context root (like WebSphere), see {@link #init(ServletContext)} for more details.</li>
 * <li>Some containers (like Jetty) unpack the war file to a different directory at server start which means that files are gone in temp.</li>
 * </ul>
 * </p>
...
public class ContextConnector extends AbstractLocalFileSystemConnector {
	...
	/**
	 * {@inheritDoc} This method will prepare the connector for further usage, additionally it will check if the aforementioned drawback exists. It will try to resolve the default
	 * {@link PropertiesLoader#getUserFilesPath() UserFilesPath} with <code><a href="http://tomcat.apache.org/tomcat-5.5-doc/servletapi/javax/servlet/ServletContext.html#getRealPath(java.lang.String)"
	 * target="_blank">ServletContext.getRealPath(String)</a></code> against the local filesystem (real path). If it fails, it will emit helpful log messages and will throw an exception too.
	 */
	public void init(final ServletContext servletContext) throws Exception {
		this.servletContext = servletContext;
		final String defaultAbsolutePath = this.getRealUserFilesAbsolutePath(PropertiesLoader.getUserFilesPath());
