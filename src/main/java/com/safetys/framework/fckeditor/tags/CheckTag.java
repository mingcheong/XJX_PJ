...
public class CheckTag extends TagSupport {
	...
	public void setCommand(String command) throws JspTagException {
		if (!CheckTag.commands.contains(command)) { throw new JspTagException("You have to provide one of the following commands: " + CheckTag.commands); }
	...
	public int doStartTag() throws JspException {
		...
		if (this.command.equals(CheckTag.FILE_UPLOAD)) {
		...
		if (this.command.equals(CheckTag.GET_RESOURCES)) {
		...
		if (this.command.equals(CheckTag.CREATE_FOLDER)) {
		...
		if (this.command.equals(CheckTag.COMPATIBLE_BROWSER)) {
