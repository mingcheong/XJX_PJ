...
public class Command {
	...
	private static final Map<String, Command> getCommands = new HashMap<String, Command>(3);
	private static final Map<String, Command> postCommands = new HashMap<String, Command>(2);
	...
	static {
		...
		Command.getCommands.put(Command.GET_FOLDERS.getName(), Command.GET_FOLDERS);
		Command.getCommands.put(Command.GET_FOLDERS_AND_FILES.getName(), Command.GET_FOLDERS_AND_FILES);
		Command.getCommands.put(Command.CREATE_FOLDER.getName(), Command.CREATE_FOLDER);

		// initialize the post commands
		Command.postCommands.put(Command.FILE_UPLOAD.getName(), Command.FILE_UPLOAD);
		Command.postCommands.put(Command.QUICK_UPLOAD.getName(), Command.QUICK_UPLOAD);
	}
	...
	public static Command valueOf(final String name) {
		...
		if (command == null) { throw new IllegalArgumentException("No command const " + name); }

		return command;
	}

	/**
	 * Returns <code>true</code> if name represents a valid <code>GET</code> command constant.
	 * 
	 * @param name
	 *            the command to check
	 * @return <code>true</code> if name represents a valid command, else <code>false</code>
	 */
	public static boolean isValidForGet(final String name) {
		return Command.getCommands.containsKey(name);
	}

	/**
	 * Returns <code>true</code> if name represents a valid <code>POST</code> command constant.
	 * 
	 * @param name
	 *            the command to check
	 * @return <code>true</code> if name represents a valid command, else <code>false</code>
	...
	public static boolean isValidForPost(final String name) {
		return Command.postCommands.containsKey(name);
	}

	/**
	 * Returns the command constant with the specified name. In contrast to {@link #valueOf(String)} it returns a null instead of throwing an exception if a command constant was not found.
	 * 
	 * @param name
	 *            the name of the constant to return
