...
public class UtilsFile {
	...
	public static String sanitizeFileName(final String filename) {
		...
		if (Utils.isEmpty(filename)) { return filename; }

		final String name = PropertiesLoader.isForceSingleExtension() ? UtilsFile.forceSingleExtension(filename) : filename;
	...
	public static String sanitizeFolderName(final String folderName) {
		...
		if (Utils.isEmpty(folderName)) { return folderName; }
	...
	public static boolean isValidPath(final String path) {
		if (Utils.isEmpty(path)) { return false; }

		if (UtilsFile.ILLEGAL_CURRENT_FOLDER_PATTERN.matcher(path).find()) { return false; }
