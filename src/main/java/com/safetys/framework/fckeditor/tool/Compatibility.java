...
public class Compatibility {
	...
	public static boolean check(final String userAgentString) {
		if (Utils.isEmpty(userAgentString)) { return false; }

		final String userAgentStr = userAgentString.toLowerCase();

		// IE 5.5+, check special keys like 'Opera' and 'mac', because there are
		// some
		// other browsers, containing 'MSIE' in there agent string!
		if ((userAgentStr.indexOf("opera") < 0) && (userAgentStr.indexOf("mac") < 0) && (Compatibility.getBrowserVersion(userAgentStr, ".*msie ([\\d]+.[\\d]+).*") >= 5.5f)) { return true; }

		// for all gecko based browsers
		if (Compatibility.getBrowserVersion(userAgentStr, ".*rv:([\\d]+.[\\d]+).*") > 1.7f) { return true; }

		// Opera 9.5+
		if ((Compatibility.getBrowserVersion(userAgentStr, "opera/([\\d]+.[\\d]+).*") >= 9.5f) || (Compatibility.getBrowserVersion(userAgentStr, ".*opera ([\\d]+.[\\d]+)") >= 9.5f)) { return true; }

		// Safari 3+
		if (Compatibility.getBrowserVersion(userAgentStr, ".*applewebkit/([\\d]+).*") >= 522f) { return true; }
